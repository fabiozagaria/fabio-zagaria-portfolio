...
describe("portfolio projects", () => {
  ...
  it("links both repositories for the expense tracker", () => {
    const expenseTracker = PROJECTS.find(
      (project) => project.id === "gestionale-spese",
    );

    expect(
      expenseTracker?.repositories.map((repository) => repository.label),
  ...
  it("labels Task Manager honestly as a guided learning lab", () => {
    const taskManager = PROJECTS.find(
      (project) => project.id === "task-manager-security-lab",
    );

    expect(taskManager?.status).toBe("Laboratorio attivo");
    expect(taskManager?.focus).toContain("Laboratorio guidato");
    expect(taskManager?.statusDetail).toContain(
      "non un prodotto ideato autonomamente",
    );
